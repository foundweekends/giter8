...
class IntegrationTest extends AnyFlatSpec with IntegrationTestHelpers with Matchers {
  ...
  it should "handle booleans" in testCase { case (template, expected, actual) =>
    """foo = now you
        |show = yes
        |hide = false
  ...
  it should "allow .g8ignore to take precedence over .gitignore" in testCase { case (template, expected, actual) =>
  ...
  it should "not ignore default files if .g8ignore is present" in testCase { case (template, expected, actual) =>
    val fileList = Seq(
      "giter8.sbt",
      "g8.sbt",
  ...
  it should "not try to render files which match verbatim rules" in testCase { case (template, expected, actual) =>
    """
        |verbatim = foo bar.bar baz/ !foo.bar"
        |test = foo
      """.stripMargin >> (template / "src" / "main" / "g8" / "default.properties")
    Seq(template / "src" / "main" / "g8", expected).foreach { base =>
      "$test" >> (base / "foo")
      "$test" >> (base / "baz" / "spoon")
      "$test" >> (base / "bar.bar")
    }
  ...
  it should "apply multiple filters to files and directories" in testCase { case (template, expected, actual) =>
    """
        | test = Foo Bar
      """.stripMargin >> (template / "src" / "main" / "g8" / "default.properties")
