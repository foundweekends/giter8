...
If the value of a field is one of `y`, `yes`, `n`, `no`, `true`, or
`false`, the field is assumed to be a boolean and can be used as a
...
Please notice that if you use a boolean field as string in a template
it will be rendered as `"true"` or `"false"`, regardless of user input.
