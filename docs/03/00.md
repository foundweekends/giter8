...
If the value of field `foo` is one of `y`, `yes`, `n`, `no`, `true`, or
`false`, a companion field `__foo` of type boolean will be automatically
be added. It can be then used in [StringTemplate conditional][conditionals]:
