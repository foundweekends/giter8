...
템플릿 리파짓토리는 Github 상에서는 `.g8` 로 확장자가 끝나도록 규약을 만들었습니다.
...
Giter8에서는 이것을 `unfiltered/unfiltered.g8` 라는 리파짓토리로 인식하고, 프로젝트 템플릿 및 매개 변수를 받습니다. 
그 밖에, git 리파짓토리 주소를 전체 이름으로 지정하는 것도 가능합니다.
...
Giter8 는 Git과 마찬가지로, ssh 키를 사용하여, 개인 개인 리파짓토리에 엑세스 할수 있습니다.