...
object Scripted {
  ...
  def scriptedTask: Initialize[InputTask[Unit]] = Def.inputTask {
    ...
    (scriptedDependencies, scriptedTests, scriptedRun, sbtTestDirectory, scriptedBufferLog, scriptedSbt, scriptedScalas, sbtLauncher) map {
      (deps, m, r, testdir, bufferlog, version, scriptedScalas, launcher) =>
  ...
  lazy val scriptedSettings: Seq[Def.Setting[_]] = Seq(
